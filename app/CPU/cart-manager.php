...
class CartManager
{
    ...
    public static function add_to_cart($request, $from_api = false)
    {
        ...
        if ($request->has('color')) {
            $str = Color::where('code', '#'.str_replace('#','',$request['color']))->first()->name;
    ...
    public static function add_to_room_cart($request, $from_api = false)
    {
        $str = '';
        $variations = [];
        $price = 0;
        ...
        $user = Helpers::get_customer($request);
        $room = RoomDesign::find($request->id);
        ...
        //Gets all the choice values of customer choice option and generate a string like Black-S-Cotton
        $choices = [];
        foreach (json_decode($room->choice_options) as $key => $choice) {
            $choices[$choice->name] = $request[$choice->name];
            $variations[$choice->title] = $request[$choice->name];
            if ($str != null) {
                $str .= '-' . str_replace(' ', '', $request[$choice->name]);
        ...
        $cart['color']          = $request->has('color') ? $request['color'] : null;
        $cart['product_id']     = $room->id;
        $cart['product_type']   = 'room';
        $cart['choices']        = json_encode($choices);

        //chek if out of stock
        if (($room['product_type'] == 'physical') && ($room['current_stock'] < $request['quantity'])) {
            return [
                'status' => 0,
                'message' => translate('out_of_stock!')
            ];
        ...
        //Check the string and decreases quantity for the stock
        if ($str != null) {
            $count = count(json_decode($room->variation));
            for ($i = 0; $i < $count; $i++) {
                if (json_decode($room->variation)[$i]->type == $str) {
                    $price = json_decode($room->variation)[$i]->price;
                    if (json_decode($room->variation)[$i]->qty < $request['quantity']) {
                        return [
                            'status' => 0,
                            'message' => translate('out_of_stock!')
                        ];
        ...
        $tax = Helpers::tax_calculation($price, $room['tax'], 'percent');

        //generate group id
        if ($user == 'offline') {
            $check = session('offline_cart');
            $cart_check = $check->where('seller_id', ($room->added_by == 'admin') ? 1 : $room->user_id)
                ->where('seller_is', $room->added_by)->first();
        } else {
            $cart_check = Cart::where([
                'customer_id' => $user->id,
                'seller_id' => ($room->added_by == 'admin') ? 1 : $room->user_id,
                'seller_is' => $room->added_by])->first();
        }
        ...
        $cart['price'] = $price;
        $cart['tax'] = $tax;
        $cart['tax_model'] = $room->tax_model;
        $cart['slug'] = $room->slug;
        $cart['name'] = $room->name;
        $cart['discount'] = Helpers::get_product_discount($room, $price);
        /*$data['shipping_cost'] = $shipping_cost;*/
        $cart['thumbnail'] = $room->thumbnail;
        $cart['seller_id'] = ($room->added_by == 'admin') ? 1 : $room->user_id;
        $cart['seller_is'] = $room->added_by;
        $cart['shipping_cost'] = $room->product_type == 'physical' ? CartManager::get_shipping_cost_for_product_category_wise($room,$request['quantity']):0;
        // if ($room->added_by == 'seller') {
        //     $cart['shop_info'] = Shop::where(['seller_id' => $room->user_id])->first()->name;
        // } else {
            $cart['shop_info'] = Helpers::get_business_settings('company_name');
        // }
        ...
        if($shippingMethod == 'inhouse_shipping')
        {
            ...
            if($room->added_by == 'admin'){
                $admin_shipping = ShippingType::where('seller_id',0)->first();
                $shipping_type = isset($admin_shipping)==true?$admin_shipping->shipping_type:'order_wise';
            }else{
                $seller_shipping = ShippingType::where('seller_id',$room->user_id)->first();
                $shipping_type = isset($seller_shipping)==true? $seller_shipping->shipping_type:'order_wise';
            }
        }
        $cart['shipping_type']=$shipping_type;
