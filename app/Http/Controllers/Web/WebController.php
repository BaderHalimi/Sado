...
class WebController extends Controller
{
    ...
    public function roomDesign() {
        $products = Product::whereNotNull('model')->latest()->get();
        ...
        return view('roomDesign.roomDesign', compact('products', 'data', 'categories'));
    }
    ...
    public function roomList() {
        ...
        //     "shipper_address": {
        //       "name": "Jane Smith",
        //       "company": "Clothing retailer ltd.ew",
        //       "address1": "Great Russel st. 5 23 23",
        //       "address2": "",
        //       "address3": "",
        //       "city": "Londonasd345",
        //       "state": "London234",
    ...
    public function roomView($id) {
        $room = RoomDesign::where([
            'id' => $id
        ])->first();
        $user_id = auth('customer')->id();
        $product_id = $room->product_id;

        $bought_product = Product::whereHas('order_details.order', function ($query) use ($user_id) {
            $query->where('customer_id', $user_id);
        })->where('id', $product_id)->first();

        // dd($bought_product);

        $products = Product::whereNotNull('model')->active()->latest()->get();

        $isRoomBuy = UserRoom::where([
            'user_id' => $user_id,
            'room_id' => $id
        ])->get()->count();
        $isRock = $bought_product == null? 1 : 0;

        $categories = Category::where(['parent_id' => 0])->get();

        $data = [
            "defaultRoomData" => $room["model"],
            "user_id" => $user_id,
            "isRoomLock" => $isRock,
            "data" => 1,
            "roomId" => $id,
    ...
    public function addRoom(Request $request) {
        ...
        $room->name = $name;
        $room->slug = $name;
        $room->user_id = auth('customer')->id();
        $room->user_email = auth('customer')->user()->email;

        $room_default_price = BusinessSetting::where('type', 'sales_commission_price')->first()->value;
        $room_commision_rate = BusinessSetting::where('type', 'sales_commission_room')->first()->value;

        $room->shipping_cost = $room_default_price * $room_commision_rate / 100;

        $room->unit_price = $room_default_price;
        $result = $room->save();
        ...
        if ($is_base64) {
            ...
            $filename = $id . '.png';
            $url = 'storage/app/public/product/thumbnail/' . $filename;
        ...
        if($result) {
            return response()->json([
                'status' => 1,
                'message' => 'Add Room Design for sale',
            ]);
        }
    ...
    public function searched_room_products(Request $request)
    {
        $request->validate([
            'name' => 'required',
        ], [
        ...
        return response()->json([
            'result' => view(VIEW_FILE_NAMES['room_search_result'], compact('products'))->render(),
        ]);
