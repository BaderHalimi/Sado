...
class ProductListController extends Controller
{
    ...
    public function default_theme($request)
    {
        ...
        // $product_data = Product::active()->with(['reviews']);
        // $query = Product::active()->with(['reviews']);
        if ($request->input('data_from') == 'category' && $request->has('id')) {
            $categories = $request->input('categories', []);
            $categories[] = $request->input('id');
            $request->merge(['categories' => $categories]); // دمج البيانات الجديدة في الطلب
        }
        if ($request['data_from'] == 'shop' && !empty($request['id'])) {
            $query = Product::active()->with(['reviews'])->where('user_id', $request['id']);
        } else {
            $query = Product::active()->with(['reviews']);
        }
        if (isset($request['text']) && !empty($request['text']) && $request['text'] != '') {
            $query = $query->where('name', 'LIKE', '%' . $request['text'] . '%');
        }
        if (!empty($request['categories'])) {
            $product_ids = Product::all()->filter(function ($product) use ($request) {
                return collect(json_decode($product['category_ids'], true))->pluck('id')->intersect($request['categories'])->isNotEmpty();
            })->pluck('id');
        ...
        if (!empty($request['colors'])) {
            $product_ids = Product::all()->filter(function ($product) use ($request) {
                return collect(json_decode($product['colors'], true))->pluck('id')->intersect($request['colors'])->isNotEmpty();
            })->pluck('id');
        ...
        if ($request['data_from'] == 'top-rated') {
            ...
            $featured_deal_product_ids = FlashDealProduct::where('flash_deal_id', FlashDeal::where(['status' => 1, 'deal_type' => 'feature_deal'])->pluck('id')->first())->pluck('product_id');
            $query = $query->whereIn('id', $featured_deal_product_ids);
        ...
        if ($request['min_price'] || $request['max_price']) {
            $query = $query->whereBetween('unit_price', [Helpers::convert_currency_to_usd($request['min_price']), Helpers::convert_currency_to_usd($request['max_price'])]);
        }
