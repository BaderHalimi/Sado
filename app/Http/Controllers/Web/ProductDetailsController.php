...
class ProductDetailsController extends Controller
{
    ...
    {
        $product = Product::active()->with(['reviews', 'seller.shop', 'comments.user', 'comments.replies.user'])->where('slug', $slug)->first();
        ...
        if ($product != null) {
            ...
            $top_rated = Review::where('product_id', $product->id)
                ->whereNotNull('comment')
                ->orderBy('rating', 'desc') // ترتيب تنازلي للحصول على أعلى تقييمات
                ->take(2) // جلب أول تقييمين فقط
                ->get(); // تنفيذ الاستعلام
            $reviews = Review::where('product_id', $product->id)
                ->orderBy('rating', 'desc') // ترتيب تنازلي للحصول على أعلى تقييمات
                ->lazy(); // تنفيذ الاستعلام
            foreach ($product->reviews as $rev) {
                $rating = $rev['rating'];
                if ($rating == 5) {
                    $rates_count['five_stars']++;
                } elseif ($rating == 4) {
                    $rates_count['four_stars']++;
                } elseif ($rating == 3) {
                    $rates_count['three_stars']++;
                } elseif ($rating == 2) {
                    $rates_count['two_stars']++;
                } elseif ($rating == 1) {
                    $rates_count['one_star']++;
                }
            }
    ...
    public function storeComment(Request $request)
    {
        ...
        if (!Auth::check()) {
            Toastr::error(translate('You must be logged in to comment.'));
            return back();
        }

        $comment = new Comment();
        $comment->customer_id = auth('customer')->id(); // استخدم customer_id هنا
        $comment->product_id = $request->product_id;
        $comment->comment = $request->comment;
        $comment->save();

        Toastr::success(translate('Comment added successfully!'));
        return back();
    }
    ...
    public function addComment(Request $request)
    {
        ...
        Comment::create([
            ...
            'customer_id' => auth('customer')->id() ?? 0, // استخدم customer_id هنا
    ...
    {
        $product = Product::with('comments.replies')->findOrFail($id);
        return view('web-views.partials.product-comments', compact('product'))->render();
    }
    // في ProductDetailsController.php
    ...
    public function destroyComment($id)
    {
        ...
        // تحقق إذا كان المستخدم الحالي هو صاحب التعليق
    ...
    public function destroyReply($id)
    {
        ...
        // تحقق إذا كان المستخدم الحالي هو صاحب الرد
