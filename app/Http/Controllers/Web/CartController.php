  ...
  public function checkShopStatus(Request $request)
    {
        ...
        foreach ($cartItems as $cartItem) {
            if ($cartItem->product->added_by == 'seller') {
                $shop = $cartItem->product->seller->shop;
                if ($shop->temporary_close || ($shop->vacation_status && now() >= $shop->vacation_start_date && now() <= $shop->vacation_end_date)) {
                    $shopClosed = true;
                    break;
                }
            } elseif ($cartItem->product->added_by == 'admin') {
    $inhouseSettings = Helpers::get_business_settings('inhouse_shop_settings');

    if ($inhouseSettings && ($inhouseSettings['temporary_close'] || 
        ($inhouseSettings['vacation_status'] && now() >= $inhouseSettings['vacation_start_date'] && now() <= $inhouseSettings['vacation_end_date']))) {
        $shopClosed = true;
        break;
    }
}
        ...
        return response()->json(['shop_closed' => $shopClosed]);
    }
    ...
    public function addToRoomCart(Request $request)
    {
        // dd($request["id"]);
        $cart = CartManager::add_to_room_cart($request);
        session()->forget('coupon_code');
        session()->forget('coupon_type');
        session()->forget('coupon_bearer');
        session()->forget('coupon_discount');
...
public function updateQuantity_guest(Request $request)
{
    try {
        ...
        return response()->json(['status' => 0, 'message' => 'Internal Server Error', 'error' => $e->getMessage()]);
    }
}
