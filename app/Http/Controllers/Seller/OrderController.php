...
use App\CPU\Convert;
use Carbon\Carbon;
use App\Services\StarLinksService;
...
public function createOrderShipment(Request $request)
{
    ...
    // تحديد نقطة النهاية المطلوبة
    $endpoint = $request->has('shipsy') ? 'shipsy' : 'shipments';
    $shipmentDetails = $this->prepareShipmentDetails($order, $endpoint);

    // تسجيل بيانات الطلب
    \Log::info('Shipment details', $shipmentDetails);

    try {
        // إنشاء طلب HTTP باستخدام Guzzle
        $client = new Client();
        $response = $client->post("https://starlinksapi.app/api/v1/{$endpoint}", [
            'headers' => [
...
protected function prepareShipmentDetails($order, $endpoint)
{
    $shippingAddressData = json_decode($order->shipping_address_data, true);
    $billingAddressData = json_decode($order->billing_address_data, true);
    $seller = Seller::find($order->seller_id);
    $customer = User::find($order->customer_id);

    // احصل على تفاصيل المنتج من order_details
    ...
    foreach ($orderDetails as $detail) {
        ...
        // استخراج الفئات
        $categoryIds = json_decode($productDetails['category_ids'], true);
        ...
        $parcels[] = [
            ...
            'products' => [
                [
                    ...
                    'parcel_item_extra' => [
                        'extra_field_1' => $product->extra_field_1 ?? '',
                        'extra_field_2' => $product->extra_field_2 ?? '',
                        'extra_field_3' => $product->extra_field_3 ?? ''
                    ]
                ]
            ]
        ];
    ...
    if ($endpoint == 'shipments') {
        return [
            'is_test' => 'true',
        'api_key' => 'd764544bef3a30e3469478242ec8ebe1',
        'order_reference' => (string) $order->id,
        'invoice' => (string) $order->id,
        'description' => implode(', ', $productNames), // وصف الشحنة يتضمن أسماء المنتجات
        'value' => $order->order_amount,
        'export_entry_number'=> 'QS12344411DN',
        'incoterm'=> 'DDU',
        'currency' => 'SAR',
         'hs_code'=> "51051000",
        'cod_value' => $order->payment_method == 'cash_on_delivery' ? $order->order_amount : 0,
        'cod_currency' => 'SAR',
        'is_dangerous' => "1",
        'category' => $categoryString ?? 'general', // استخدام قيمة افتراضية
        'service' => [
            'service_code' => 'SA_DOM',
            'label_format' => 'ZPL300',
            'display_id' => 'my_internal_id',
            'pudo_id' => 'A100'
        ],
        'consignee_address' => [
            'name' => $shippingAddressData['contact_person_name'],
            'company' => '',
            ...
            'email' => $customer->email, // جلب البريد الإلكتروني من معلومات العميل
        ],
        'shipper_address' => [
            'name' => $seller->f_name . ' ' . $seller->l_name,
            'company' => $seller->shop->name,
            'address1' => $seller->shop->address,
            'city' => "Riyadh", // يمكنك ضبط المدينة من معلومات البائع إذا كانت متاحة
            'state' => "Riyadh", // يمكنك ضبط الولاية من معلومات البائع إذا كانت متاحة
            'zip' => $seller->zip,
            'country' => 'SA',
            'phone' => $seller->phone,
            'email' => $seller->email,
            ...
            'parcels' => $parcels,
            'collection_auto_confirm' => true
        ];
    } else if ($endpoint == 'shipsy') {
        return [
            ...
            "invoice_date" => date('Y-m-d'), // تأكد من تعيين التاريخ المناسب
            "declared_value" => $order->order_amount,
            "declared_value_without_tax" => $order->order_amount,
            "currency" => "SAR",
            "product_code" => "PL00123345",
            "num_pieces" => "10",
            ...
            "courier_partner_reference_number" => "301458690",
            "hub_code" => "FB100",
            "inco_terms" => "DDU",
            "notes" => "Testing Note",
            "reference_image_url" => "https://shipsy-public-assets.s3.amazonaws.com/shipsyflamingo/logo.png",
            "constraint_tags" => ["Chilled"],
            "customs_value" => [
                "amount" => "100",
                "currency" => "USD"
            ],
            "additional_properties" => [
                "ioss_number" => "FR275815987",
                "ad_code" => "1234567890123",
            ...
            "tax_details" => [
                ...
                "igst" => '0.0',
                "total_tax" => '0.0',
                "sender_gstin" => '22AAAAA0000A1Z4',
                "receiver_gstin" => '22AAAAA0000A1Z4'
            ],
            "delivery_time_slot_start" => '2021-11-12 14:05:36',
            "delivery_time_slot_end" => '2021-10-30 09:55:40',
            "scheduled_at" => '2021-11-12',
            "service_time" => 1200,
            "origin_details" => [
            ...
            "pieces_detail" => [
                [
                    ...
                    'declared_value' => '29.99',
                    'volume' => '100',
                    'weight' => '5.85',
                    'height' => '1', // إضافة ارتفاع افتراضي
                    'length' => '1', // إضافة طول افتراضي
                    'width' => '1', // إضافة عرض افتراضي
                ]
            ]
        ];
    }
    ...
    public function trackOrderShipment($trackingNumber)
    {
        $response = $this->starLinksService->trackShipment($trackingNumber);
...
//        return view('seller-views.order.invoice',compact('order', 'seller', 'company_phone', 'company_name', 'company_email', 'company_web_logo'));
    ...
    // تسجيل حالة الطلب قبل التأكيد
    \Log::info('Order status before confirmation', ['order_status' => $request->order_status]);

        
         // إرسال الشحنة عند تأكيد الطلب
        if ($request->order_status == 'confirmed') {
            $shipmentDetails = [
                'order_id' => $order->id
            ];
        \Log::info('Creating shipment for order', ['order_id' => $order->id]);

            $this->createOrderShipment(new Request($shipmentDetails));
        }
