...
class OrderController extends Controller
{
 ...
 private function qrcode_base64_encode($seller, $registration_number, $invoice_datetime, $invoice_amount, $invoice_tax) {
        $result = chr(1) . chr(strlen($seller)) . $seller;
        $result .= chr(2) . chr(strlen($registration_number)) . $registration_number;
        $result .= chr(3) . chr(strlen($invoice_datetime)) . $invoice_datetime;
        $result .= chr(4) . chr(strlen($invoice_amount)) . $invoice_amount;
        $result .= chr(5) . chr(strlen($invoice_tax)) . $invoice_tax;
...
public function show_invoice($id)
{
    ...
    if (!$order) {
        ...
        // Redirect to the error page or take appropriate action
    ...
    $company_name = BusinessSetting::where('type', 'company_name')->first()->value;
    $company_web_logo = BusinessSetting::where('type', 'company_web_logo')->first()->value;
    $registration_number = "310574808900003";
    $date = date('d/M/Y ' . config('timeformat'), strtotime($order->created_at));

    // Generate the QR code result
    $result = $this->qrcode_base64_encode(
        $order->details->first()->seller->shop->name ?? 'sado',
        $order->details->first()->seller->gst ?? '',
        $date,
            BackEndHelper::format_currency($order->order_amount),
            $order->order_amount * 0.15
        );

    // Debugging: Output the result
    // dd($order->details->first()->seller->gst);

    // Pass the necessary data to the view
    return view('admin-views.order.show-invoice', compact('order','result', 'company_phone', 'company_name', 'company_email', 'company_web_logo', 'registration_number', 'date'));
}
...
public function generate_invoice($id)
{
    ...
    $order = Order::with('seller')
        ...
        ->with('details')
        ->where('id', $id)
        ->first();  // Use first() to retrieve a single order
...
$result = $this->qrcode_base64_encode(
    ...
    $mpdf_view = View::make('admin-views.order.invoice',
        compact('order', 'seller', 'company_phone', 'company_name', 'company_email', 'company_web_logo','result')
    );

    // Call your function after $order is defined
