...
class SystemController extends Controller
{
    ...
    public function choose_shipping_address_other(Request $request){
        ...
        if ($physical_product === 'yes') {
            if (empty($shipping['first_name']) || empty($shipping['last_name']) || empty($shipping['phone']) || empty($shipping['email'])) {
        ...
        if (!empty($shipping)) {
            // if (isset($shipping['save_address']) && $shipping['save_address'] === 'on') {
                $address_id = ShippingAddress::insertGetId([
                    ...
                    'contact_person_name' => $shipping['first_name'].' '.$shipping['last_name'],
                    // 'address' => $shipping['address'],
                    'phone' => $shipping['phone'],
                    'email' => $is_guest ? $shipping['email'] : null,
                    // 'address' => $billing['billing_address'],
                    // 'city' => $billing['billing_city'],
                    // 'zip' => $billing['billing_zip'],
                    // 'country' => $billing['billing_country'],
        ...
        // التحقق من صحة بيانات الفوترة
        $billing_address_id = $address_id;
        if ($request->billing_addresss_same_shipping == 'false' && $billing_input_by_customer) {
            // return response()->json(['errors' => $billing['billing_country']], 403);
            if (empty($billing['billing_address']) || empty($billing['billing_city']) || empty($billing['billing_zip']) || empty($billing['billing_country'])) {
        ...
        session()->put('address_id', $address_id);
        session()->put('billing_address_id', $billing_address_id);
        // return response()->json(['errors' =>session('billing_address_id')], 403);
        // dd(session('billing_address_id'));
    ...
    //     if(isset($shipping['shipping_method_id'])) {
    //         if ($shipping['first_name'] == null || $shipping['last_name'] == null ||  $shipping['phone'] == null || $shipping['email'] == null || ($is_guest && $shipping['email'] == null)) {
    //             return response()->json([
    //                 'errors' => translate('Fill_all_required_fields_of_shipping_address')
    //             ], 403);
    //         // } elseif ($country_restrict_status && !self::delivery_country_exist_check($shipping['country'])) {
