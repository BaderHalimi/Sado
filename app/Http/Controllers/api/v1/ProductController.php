...
use App\Model\Reply; // تأكد من أن هذا هو المسار الصحيح لكلاس Reply
use Carbon\Carbon;
...
public function addComment(Request $request)
{
    try {
        ...
        // تحقق إذا كان المستخدم مسجل دخول أو لا
    $customerId = auth('customer')->check() ? auth('customer')->id() : 0; // تعيين 0 للزائر
...
public function addReply(Request $request)
{
    ...
    // تحقق مما إذا كان المستخدم مسجل الدخول أو زائر
    ...
    return response()->json(['message' => translate('Reply added successfully!')], 200);
}
...
public function likeComment(Request $request)
{
    ...
    // التحقق إذا كان المستخدم مسجل دخوله، واستخدام 0 كـ customer_id إذا كان زائرًا
    $customerId = auth('customer')->id() ?? 0;
    ...
    if ($like) {
        // إذا كان المستخدم قد أحب التعليق بالفعل، قم بإزالة الإعجاب
        $like->delete();
        return response()->json(['message' => 'Comment unliked successfully!']);
    } else {
        // إذا لم يكن المستخدم قد أحب التعليق بعد، أضف إعجاب
        CommentLike::create([
...
public function destroyComment($id)
{
    try {
        ...
        if (auth('customer')->id() == $comment->customer_id) {
            $comment->delete();
...
public function destroyReply($id)
{
    ...
    if (auth('customer')->id() == $reply->customer_id) {
        $reply->delete();
...
{
    try {
        \Log::info("Fetching comments for product ID: {$id}");
        
        $product = Product::with(['comments.user', 'comments.replies.user'])->findOrFail($id);

        \Log::info("Product found: {$product->name}");

        $comments = $product->comments->map(function ($comment) {
            $userName = $comment->user ? $comment->user->f_name . ' ' . $comment->user->l_name : 'Guest'; // استخدام f_name و l_name

            return [
                'id' => $comment->id,
                'product_id' => $comment->product_id,
                'customer_id' => $comment->customer_id,
                'user_name' => $comment->customer_id == 0 ? 'walking customer' : $userName, // استخدم الاسم الكامل أو 'walking customer' للزوار
                'comment' => $comment->comment,
                'parent_id' => $comment->parent_id,
                'is_liked' => auth('customer')->check() ? $comment->likes()->where('customer_id', auth('customer')->id())->exists() : false,
                'like_count' => $comment->likes()->count(),
                'created_at' => $comment->created_at,
                'updated_at' => $comment->updated_at,
                'replies' => $comment->replies->map(function ($reply) {
                    $replyUserName = $reply->user ? $reply->user->f_name . ' ' . $reply->user->l_name : 'Guest'; // استخدام f_name و l_name للردود

                    return [
                        'id' => $reply->id,
                        'product_id' => $reply->product_id,
                        'customer_id' => $reply->customer_id,
                        'user_name' => $reply->customer_id == 0 ? 'walking customer' : $replyUserName, // استخدم الاسم الكامل أو 'walking customer' للزوار
                        'comment' => $reply->comment,
                        'parent_id' => $reply->parent_id,
                        'is_liked' => auth('customer')->check() ? $reply->likes()->where('customer_id', auth('customer')->id())->exists() : false,
        ...
        \Log::info("Comments fetched successfully for product ID: {$id}");

        return response()->json($comments, 200);
    } catch (\Exception $e) {
        \Log::error("Error fetching comments for product ID {$id}: " . $e->getMessage());
