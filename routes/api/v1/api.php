...
Route::group(['prefix' => 'products'], function () {
    // مسار لجلب التعليقات بدون الحاجة للمصادقة
    Route::get('comments/{id}', 'ProductController@getComments');

    // مسارات للتعليقات والردود تتطلب المصادقة أو تكون مفتوحة للزوار
    Route::post('add-comment', 'ProductController@addComment'); // يمكن جعله مفتوحًا للزوار
    Route::post('add-reply', 'ProductController@addReply'); // يمكن جعله مفتوحًا للزوار
    Route::post('like-comment', 'ProductController@likeComment'); // يمكن جعله مفتوحًا للزوار
    Route::delete('destroy-comment/{id}', 'ProductController@destroyComment')->middleware('auth:api');
    Route::delete('destroy-reply/{id}', 'ProductController@destroyReply')->middleware('auth:api');
});
