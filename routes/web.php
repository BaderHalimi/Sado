...
Route::group(['namespace' => 'Web', 'middleware' => ['maintenance_mode', 'guestCheck']], function () {
    ...
    Route::get('searched-room-products', 'WebController@searched_room_products')->name('searched-room-products');
    ...
    Route::any('checkout-details', 'WebController@checkout_details')->name('checkout-details');
    ...
    // 3D room design part
    Route::get('room-design', 'WebController@roomDesign')->name('room-design');
    Route::get('room-list', 'WebController@roomList')->name('room-list');
    Route::get('room{id}', 'WebController@roomView')->name('room-view');
    Route::post('add-room', 'WebController@addRoom')->name('add_room');
    ...
    Route::post('/product/add-comment', 'ProductDetailsController@addComment')->name('web.product.add-comment');
    Route::get('/product/comments/{id}', 'ProductDetailsController@getComments')->name('web.product.comments');
    // في web.php
    Route::post('/product/add-reply', 'ProductDetailsController@addReply')->name('web.product.add-reply');
    // في web.php
    Route::post('/product/like-comment', 'ProductDetailsController@likeComment')->name('web.product.like-comment');
    Route::delete('/comments/{id}', 'ProductDetailsController@destroyComment')->name('comments.destroy');
    Route::delete('/replies/{id}', 'ProductDetailsController@destroyReply')->name('replies.destroy');
...
Route::group(['prefix' => 'cart', 'as' => 'cart.', 'namespace' => 'Web'], function () {
    ...
    Route::post('addRoom', 'CartController@addToRoomCart')->name('addRoom');
    ...
    Route::get('check-shop-status', 'CartController@checkShopStatus')->name('check-shop-status');
});
...
if (!$is_published) {
    Route::group(['prefix' => 'payment'], function () {
        ...
        Route::group(['prefix' => 'fatoorah', 'as' => 'fatoorah.'], function () {
            // للصفحة يلي تحول المستخدم للدفع
            Route::get('/pay', [MyFatoorahController::class, 'index'])->name('pay');

            // للصفحة يلي ماي فاتورة تعمل عليها callback لما يتم الدفع
            Route::post('/callback', [MyFatoorahController::class, 'callback'])->name('callback');
