...
<head>
    ...
    <style>
        ...
        * {
            font-family: 'SF Arabic Rounded', 'SF Arabic', sans-serif;
        }
...
function updateCartQuantity(minimum_order_qty, cart_id) {
    let update_quantity_url = $("#update_quantity_url").data("url");
    let token = $('meta[name="_token"]').attr("content");
    let cart_quantity_input = $(`#cartQuantity${cart_id}`);
    let product_qty = parseInt(cart_quantity_input.val());

    if (isNaN(product_qty) || product_qty < minimum_order_qty) {
        toastr.error('Quantity is invalid');
        cart_quantity_input.val(minimum_order_qty);
        return;
    }
    ...
    $.post(
        ...
        function (response) {
            if (response.status == 0) {
                ...
                cart_quantity_input.val(response.qty); // revert to the old value
            } else {
                toastr.success(response.message);
                cart_quantity_input.val(response.qty);
