...
<div class="navbar-tool dropdown {{ Session::get('direction') === 'rtl' ? 'mr-md-3' : 'ml-md-3' }}"
    ...
    <a class="btn bg-secondary px-2 py-1 px-sm-3 py-sm-3
        ...
        <span class="navbar-tool-label d-none">
            @php($cart = \App\CPU\CartManager::get_cart())
            {{ $cart->count() }}
        </span>
        <i class="navbar-tool- czi-cart text-primary" style="font-size: 16px;"></i>
    </a>
                        ...
                        <div class="widget-cart-item pb-2">
                            ...
                            <div class="media">
                                <a class="d-block {{ Session::get('direction') === 'rtl' ? 'ml-2' : 'mr-2' }}"
                                    href="{{ $cartItem['product_type'] == 'room' ? route('room-view', $cartItem['product_id']) : route('product', $cartItem['slug']) }}">
                                    <img width="64"
                                ...
                                <div class="media-body">
                                    <h6 class="widget-product-title">
                                        <a
                                            href="{{ $cartItem['product_type'] == 'room' ? route('room-view', $cartItem['product_id']) : route('product', $cartItem['slug']) }}">{{ Str::limit($cartItem['name'], 30) }}</a>
                                    </h6>
