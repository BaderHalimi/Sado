        ...
        <div class="__cart-total __cart-total_sticky">
                    ...
                    <div class="row justify-content-center g-4">
                        @foreach ($company_reliability as $key=>$value)
                            @if ($value['status'] == 1 && !empty($value['title']))
                                <div class="col-3 px-0 text-center mobile-padding">
                                    <div class="@if($key!=0) container @endif">
                                        <img class="order-summery-footer-imge" src="{{asset("/storage/app/public/company-reliability").'/'.$value['image']}}"
            ...
            <div class="mt-4">
                @if($web_config['guest_checkout_status'] || auth('customer')->check())
                    <a onclick="checkout()" class="btn btn--primary btn-block proceed_to_next_button {{$cart->count() <= 0 ? 'disabled' : ''}}" >{{translate('complete_purchase')}}</a>
                @else
                    <a href="{{route('customer.auth.login')}}" class="btn btn--primary btn-block proceed_to_next_button {{$cart->count() <= 0 ? 'disabled' : ''}}" >{{translate('complete_purchase')}}</a>
                @endif
            </div>
            {{-- @if( $cart->count() != 0)
