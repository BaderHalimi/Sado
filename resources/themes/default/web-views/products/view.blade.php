...
@section('content')
    ...
    <!-- Page Content-->
    <div class="container py-5 mb-2 mb-md-4 rtl __inline-35" dir="{{ Session::get('direction') }}">
        <div class="row">
            <!-- Sidebar-->
            <aside
                ...
                <div class="cz-sidebar __inline-35" id="shop-sidebar"
                    style="background-color: #fafbfc !important; box-shadow:0px 0px 0px #fafbfc !important;">
                    ...
                    <div class="container">
                        @include('web-views.partials._product_filters')
                    </div>
                    {{-- <div class="pb-0">
            ...
            <section class="col-lg-9">
                <!-- Page Title-->
                <div class="py-3" dir="{{ Session::get('direction') }}">
                    <div class="search-page-header border-0 shadow-0"
                        style="background-color: #fafafc !important; box-shadow: 0px 0px 0px !important;">
                        <div>
                            <h5 class="font-semibold mb-1">{{ translate(str_replace('_', ' ', $data['data_from'])) }}
                                {{ translate('products') }}
                                {{ isset($data['brand_name']) ? '(' . $data['brand_name'] . ')' : '' }}</h5>
                            <div class="view-page-item-count">{{ $products->total() }} {{ translate('items_found') }}
                            </div>
                        </div>
                        <form id="search-form" class="d-none d-lg-block" action="{{ route('products') }}" method="GET">
                            <input hidden name="data_from" value="{{ $data['data_from'] }}">
                            <div class="sorting-item"
                                style="background-color: #fafafc !important; box-shadow: 0px 0px 0px !important; border: none !important;">
                                <select class="w-100 border-0" style="background-color: #fafafc !important;"
                                    onchange="filter(this.value)">
                                    <option value="" selected disabled>{{ translate('sort_by') }}</option>
                                    <option value="latest">{{ translate('latest') }}</option>
                                    <option value="low-high">{{ translate('low_to_High_Price') }} </option>
                                    <option value="high-low">{{ translate('High_to_Low_Price') }}</option>
                                    <option value="a-z">{{ translate('A_to_Z_Order') }}</option>
                ...
                @if (count($products) > 0)
                    <div class="row gap-5" id="ajax-products">
...
@push('script')
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function filterProducts(priceCh = false) {
                text = document.getElementById('searchInputText').value;
                ...
                document.querySelectorAll("input[id^='category']:checked").forEach((el) => {
                    selectedCategories.push(el.id.replace("category", ""));
                });
                document.querySelectorAll("input[id^='brand']:checked").forEach((el) => {
                    selectedBrands.push(el.id.replace("brand", ""));
                });
                document.querySelectorAll("input[id^='color']:checked").forEach((el) => {
                    selectedColors.push(el.id.replace("color", ""));
                });
                ...
                $.get({
                    ...
                    data: {
                        id: "{{ $data['id'] }}",
                        name: "{{ $data['name'] }}",
                        data_from: "{{ $data['data_from'] ?? 'category' }}",
                        min_price: minPrice,
                        max_price: maxPrice,
                    ...
                    success: function(response) {
                        ...
                        $('.view-page-item-count').text(response.total_product +
                            ' {{ translate('items_found') }}');
                    },
                    complete: function() {
                        $('#loading').hide();
            ...
            // تحسين طريقة ربط الأحداث:
            document.getElementById("minPrice").addEventListener("change", () => filterProducts(true));
            document.getElementById("maxPrice").addEventListener("change", () => filterProducts(true));
            document.getElementById("searchInputBtn").addEventListener("click", () => filterProducts());

            document.querySelectorAll("input[id^='category'], input[id^='brand'], input[id^='color']").forEach((
                el) => {
