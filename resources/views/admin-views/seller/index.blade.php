    ...
    <form id="searchForm" action="{{ url()->current() }}" method="GET">
        <div class="input-group input-group-merge input-group-custom">
            ...
            <input id="datatableSearch_" type="search" name="search" class="form-control"
                placeholder="{{translate('search_by_Name_or_Phone_or_Email')}}" aria-label="Search orders"
                value="{{ $search }}">
...
<script>
    document.getElementById('datatableSearch_').addEventListener('input', function(event) {
        if (this.value === '') {
            document.getElementById('searchForm').submit(); // إعادة تحميل الصفحة عندما يكون البحث فارغًا
        }
    });
</script>
                        ...
                        <table
                            ...
                            @foreach($sellers as $key=>$seller)
                                <tr>
                                    ...
                                    <td>
                                        {{$seller->type == 2? "Room Designer" : "Seller"}}
                                    </td>
                                    <td>
                                        <div class="mb-1">
                          ...
                          <a href="{{ route('admin.sellers.edit', $seller->id) }}" class="btn btn-outline--primary btn-sm square-btn" title="{{ translate('edit') }}">
    <i class="tio-edit"></i>
</a>
    ...
    <a title="{{ translate('delete') }}"
       ...
       href="javascript:"
       onclick="form_alert('seller-{{$seller->id}}','{{ translate('want_to_delete_this_seller').'?' }}')">
        <i class="tio-delete"></i>
    </a>
@endif

<form action="{{ route('admin.sellers.delete', $seller->id) }}" method="post" id="seller-{{$seller->id}}">
    @csrf
    @method('delete')
</form>
                                        </div>
