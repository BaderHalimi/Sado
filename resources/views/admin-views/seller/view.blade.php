...
@section('content')
                ...
                <div class="d-flex flex-wrap gap-3 justify-content-between">
                    <div class="media flex-column flex-sm-row gap-3">
                        ...
                        <div class="media-body">
                            ...
                            @endif
                            <div class="d-block">
                                <h2 class="mb-2 pb-1">{{ $seller->shop? $seller->shop->name : translate("shop_Name")." : ".translate("update_Please") }} ({{$seller->type == 2? "Room Designer":""}})</h2>
    ...
    <span class="key text-nowrap">{{ translate('registry_Num') }}</span>
    <span>:</span>
    <span class="value">{{ $seller->shop->registry_Num ?? translate('not_available') }}</span>
</div>
<div>
    <span class="key">{{ translate('PDF_Document') }}</span>
    <span>:</span>
    @if($seller->shop->pdf_document)
        <a href="{{ asset('storage/app/' . $seller->shop->pdf_document) }}" target="_blank" class="badge badge badge-warning">{{ translate('View PDF') }}</a>
    @else
        {{ translate('not_available') }}
    @endif
</div>
<div>
    <span class="key">{{ translate('Added_tax_certificate') }}</span>
    <span>:</span>
    @if($seller->shop->pdf_documents)
        <a href="{{ asset('storage/app/' . $seller->shop->pdf_documents) }}" target="_blank" class="badge badge-info" >{{ translate('Viewـcertificate') }}</a>
    @else
        {{ translate('not_available') }}
    @endif
</div>
