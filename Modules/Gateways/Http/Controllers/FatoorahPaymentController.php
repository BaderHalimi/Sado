...
class FatoorahPaymentController extends Controller
{
    ...
    public function __construct(PaymentRequest $payment)
    {
        ...
        $this->base_url = config('services.fatoorah.base_url');
        ...
        // }
        // dd($this->api_key = $this->config_values->api_key);
    ...
    //         // ddd(response()->json($response->json()['Data']['PaymentURL'], $response->status()));
    //         return response()->json($response->json()['Data']['PaymentURL'], $response->status());
    //     } else {
    //         // ddd(response()->json($response->json()['Message'], $response->status()));
    ...
    if (!$request->has('sessionId')) {
        return response()->json('Session ID is missing in the request.', 400);
    }
    // dd(session('data'));

    // $roundedInvoiceValue = round(session('data')['payment_amount'], 3);
